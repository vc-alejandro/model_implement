...
Implementacion manual de un arbol de decision para clasificacion binaria.
No se usa ninguna libreria de aprendizaje automatico ni de estadistica
avanzada (no sklearn, no scipy). Solo se usa pandas para manejar
columnas de datos (no para el algoritmo en si) y math para el logaritmo.

Se usa entropia / ganancia de informacion como criterio de impureza,
igual que el ID3 visto en clase.

El arbol que se construye es BINARIO en cada nodo:
    - Si el atributo es numerico -> pregunta "es <= umbral?"
    - Si el atributo es categorico -> pregunta "es igual a este valor?"

Esto simplifica la implementacion (siempre hay 2 ramas: izquierda/derecha)
en vez de crear una rama por cada valor posible del atributo, como hace
el ID3 clasico visto en clase.
"""
...
def entropy(labels):
    """
    Entropia de un conjunto de etiquetas.
...
# ---------------------------------------------------------------------
# 2. GANANCIA DE INFORMACION DE UNA DIVISION (SPLIT)
# ---------------------------------------------------------------------
...
def ganancia_de_informacion(y_padre, y_izquierda, y_derecha):
    ...
    Es la reduccion de entropia que se logra al dividir y_padre
    en y_izquierda y y_derecha.
    """
    n = len(y_padre)
...
def es_columna_numerica(serie):
    """True si la columna tiene datos numericos (int o float)."""
    return serie.dtype.kind in "if"  # i = int, f = float
...
def encontrar_mejor_split(X, y, columnas):
    ...
    Para columnas numericas: prueba como umbral el punto medio
    entre cada par de valores unicos consecutivos (ordenados).

    Para columnas categoricas: prueba "es igual a este valor?"
    para cada valor unico de la columna.

    Regresa: (mejor_columna, mejor_valor, mejor_tipo, mejor_ganancia)
    """
    mejor_ganancia = 0.0
    mejor_columna = None
    mejor_valor = None
...
class DecisionTree:
    ...
    def _construir_arbol(self, X, y, columnas, profundidad):
        ...
        Regresa un nodo, que es un diccionario:

        Hoja:
            {"hoja": True, "prediccion": clase}

        Nodo interno:
            {"hoja": False, "columna": ..., "tipo": ..., "valor": ...,
             "izquierda": nodo, "derecha": nodo}
        """
...
# ---------------------------------------------------------------------
# Este bloque solo se ejecuta si corres "python decision_tree.py"
# directamente. Sirve para verificar que la implementacion es correcta,
# comparando contra los valores de Information Gain que dio el profesor
# en la diapositiva: Gain(S,Outlook)=0.246, Gain(S,Humidity)=0.151,
...
if __name__ == "__main__":
    ...
    # igual que en la diapositiva del profesor)
